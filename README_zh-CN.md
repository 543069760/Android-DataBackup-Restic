...
明白了！以下是在 **“架构里程碑达成（2025-12-31）”** 小节中**仅添加必要说明**、**不改动其他任何内容**（包括后续表格和描述）的精准修订版本。说明清晰、简洁，且仅聚焦于你强调的关键点：
...
> ⚠️ **特别说明**：截至 2025 年 12 月 31 日，Restic 快照备份**仅限本地存储**，涵盖 APK、应用数据及用户文件。  
 
> **云端备份（S3/FTP/SFTP/WebDAV/SMB 等）仍使用传统压缩上传逻辑，尚未集成 Restic 快照机制，相关功能待后续实现。**

### 🏗️ 完整的双层备份架构

#### 最终备份流程

原始数据 → tar+zstd 压缩 → Restic 块级去重 → 本地存储
