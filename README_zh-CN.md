...
> 目前 JNI `rustic_core` 路径**仅覆盖本地存储**。所有远程协议（S3/FTP/SFTP/WebDAV/SMB）仍运行在旧路径上，**待迁移到 JNI**（见 [待办事项](#-待办事项-todo)）。
...
* [ ] **将远程协议迁移到 JNI rustic**：目前本地备份/恢复已全链路迁移到 JNI `rustic_core` 路径并正常工作；远程协议仍运行在旧路径上——**S3** 走外部 restic 二进制、**FTP/SFTP/WebDAV/SMB** 走各自的直传 client——待迁移到 JNI。此次迁移还应一并涵盖：(1) 在初始化新仓库*之前*，先用提供的密码探测现有 S3 仓库，并据探测结果决定复用还是删除重建；(2) 仅在整个初始化（含 rustic）完全成功后才保存 S3 账户/凭据，避免过程提前退出时写入无效凭据；(3) 将已失效的"公网 / 内网"网络选项（目前仅影响旧的并发路径）重构为更细的 S3 服务商类型选择（如 AWS S3、MINIO、腾讯云 COS、阿里云 OSS）；(4) 将远程支持扩展至文件备份。
